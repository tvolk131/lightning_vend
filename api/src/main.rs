...
use proto::lightning_vend::lightning_vend_service_server::LightningVendServiceServer;
use service::LightningVendServiceImpl;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    Server::builder()
        ...
        .add_service(tonic_web::config().allow_all_origins().enable(
            LightningVendServiceServer::new(LightningVendServiceImpl::new()),
        ))
        .serve(address)
        .await?;
