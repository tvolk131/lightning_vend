...
fn run_command_handler(vend_coil: &State<Mutex<vend_coil::VendCoil>>) -> Result<rocket::response::content::Plain<String>, rocket::response::status::Custom<String>> {
    match vend_coil.lock().unwrap().rotate() {
        Ok(_) => Ok(rocket::response::content::Plain("Success!".to_string())),
        Err(err) => Err(rocket::response::status::Custom(rocket::http::Status::InternalServerError, format!("Failed to execute action: {}", err)))
    }
}
